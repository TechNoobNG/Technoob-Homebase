...
const UploadFile = ({ closeModal }) => {
  ...
  const handlePublish = async (e) => {
    ...
    try {
      ...
      const response = await serverApi.post("/resources/create", payload, {
        ...
        <form className="lg:mx-5 mt-7 w-full">
          ...
          <div className="mt-5 min-h-[150px]">
            <div className="w-full border-t-2 border-gray-200 mb-3">
              <div className="text-center text-xs">{uploading ? <LoaderIcon type={"spinningBubbles"} /> : <p></p>}</div>
              {placeholderImage && uploadedFile && (
                <div className="border-t-2 border-gray-200 mb-3 pt-3">
                  <div className="flex h-16">
                    <div className="flex items-start">
                      <img src={placeholderImage} alt="_img" className="w-13 h-full object-contain mr-3 p-1" />
                      <div>
                        <h1 className="truncate text-xs md:text-base w-[190px]">{fileInfo.name}</h1>
                        {fileInfo.name && (
                          <p className="text-sm text-green-500 flex flex-col-reverse md:flex-row ">
                            Upload successful ✅ <li className="text-[#7A90A7] ml-2"> {fileInfo.size / 1000}kb</li>
                          </p>
                        )}
                      </div>
                    </div>
          ...
          <div className="flex w-full justify-start items-start gap-3">
            {!uploading && placeholderImage && uploadedFile && (
              <button
